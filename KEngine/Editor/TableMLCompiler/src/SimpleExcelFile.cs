        ...
        private void ParseExcel(string filePath)
        {
            ...
            //if (IsLoadSuccess)
            {
                ...
                for (int columnIndex = 0; columnIndex < columnCount; columnIndex++)
                {
                    ...
                    ColName2Comment[colName] = HandleMultiLineComment(commentString);
                }
            }
        }

        private string HandleMultiLineComment(string commentString) {
            var str = commentString.Replace("\r\n", "\n");
            var arr = str.Split('\n');

            if (arr.Length < 2) { return commentString; }

            var paraArr = new string[arr.Length];
            for (int i = 0; i < arr.Length; i++)
            {
                paraArr[i] = string.Format("<para>{0}</para>", arr[i]);
            }
